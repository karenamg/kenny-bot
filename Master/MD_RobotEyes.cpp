...
// Return true if there is no animation happening
{
  ...
  case S_ANIMATE:  // process the next frame for this sequence
    if (_typeAnimation == RESETING){
      _typeAnimation = NORMAL;
      // Cargar la emoción NEUTRAL y obtener su tamaño de secuencia
      uint8_t seqSize = loadSequence(E_NEUTRAL);

      // Verificar si la secuencia de animación es válida y tiene al menos un marco
      if (seqSize > 0)
      {
        // Cargar el primer marco de la secuencia en cada ojo
        animFrame_t frame;
        loadFrame(&frame);
        _M->setColumn(_sd + LEFT_MODULE_OFFSET, 0, frame.eyeData[LEFT_EYE_INDEX]);
        _M->setColumn(_sd + RIGHT_MODULE_OFFSET, 0, frame.eyeData[RIGHT_EYE_INDEX]);
      }

      // Actualizar la matriz de LEDs para mostrar los cambios
      _M->control(MD_MAX72XX::UPDATE, MD_MAX72XX::ON);
    }
  ...
  case S_PAUSE: // pause this frame for the required time
    {
           ...
           (_animReverse && _animIndex < 0))
      {
        ...
        if (_typeAnimation == LOOPING) // if we're looping the animation
        {
          _animReverse = !_animReverse; // flip the direction
          _animState = S_RESTART;
          break;
        }
